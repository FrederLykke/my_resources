...
bhat - coef(lmfit1)
```
Lm() does give the same result as OLS (the difference between coefficient estimates is almost zero, except for tiny rounding errors).
...
```
Solution:
The posterior plots are showing the posterior samples from the Bayesian inference (performed with Markov chain Monte Carlo sampling). In this case the mean of the posterior distribution is used as the parameter (intercept = 2.3879, x = 0.0047, sigma = 0.3743).
...
#Add names to make it easier to interpret:
names(y_pointpred) <- c("$40k", "$80k")

#Show the point predictions:
y_pointpred
```
