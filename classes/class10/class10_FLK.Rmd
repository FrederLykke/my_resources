...
```{r}
#Calculate the OLS estimator beta hat:
#The formula is beta_hat = (X^t * X)^-1 * X^t * y
bhat <- solve(t(X) %*% X) %*% t(X) %*% y #solve() inverts the matrix, t() is the transpose
...
```{r}
#Linear regression fit using lm():
lm <- lm(y ~ x, data = data)

#Compare the coefficients from the OLS solution with the coefficients from lm():
coef(lm)

#Show bhat to see the estimators (intercept and slope):
bhat
```
...
income_range <- c(15, 300)  # Income range in thousands of dollars
sigma <- 0.4  # Error standard deviation
b <- c(2.375, 0.005)  # Intercept and slope

# Simulate household incomes uniformly
incomes <- runif(n, min = income_range[1], max = income_range[2])

# Calculate simulated GPAs
error <- rnorm(n, mean = 0, sd = sigma)
gpa <- b[1] + b[2] * incomes + error

# Create a data frame for plotting
data <- data.frame(income = incomes, gpa = gpa)

# Plot the sampled data along with regression line
ggplot(data, aes(x = income, y = gpa)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "lightblue") +  # Add regression line
  labs(x = "Household Income", y = "GPA", title = "Simulated GPA vs. Household Income") + 
  theme_minimal()
```
...
```{r}
#Using stan_glm to fit a regression line:
fit1 <- stan_glm(gpa ~ income, data = data)
print(fit1, digits=2)
...
mcmc_areas(fit1, pars = "income", prob = 0.5, prob_outer = 1) #Pernille had written "x" instead of "income"
mcmc_areas(fit1, pars = "sigma", prob = 0.5, prob_outer = 1)
```

Now let's add the fitted regression line to our plot.
```{r}
sim %>% ggplot(aes(x = x, y = gpa)) +
    geom_point() +
...
```{r}
#Use the predict() function to make point predictions:
