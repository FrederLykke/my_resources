...
For-loops are used go through a list and apply a function to each element in the list. While-loops are used to repeat a block of code as long as a condition is true.

### Ex. B: When would you use for-loops and when would you use while-loops? Explain in your own words in 1-2 sentences.
For-loops are used when you know how many times you want to repeat a block of code. While-loops are used when you want to repeat a block of code as long as a condition is true (for example for all instances (i) that are less than 6).
...
while(some_number < 20){
  print("the number is below 20, so I still run!")
...
# Men: mu (mean)=69.1, sigma (sd)=2.9
# Women: mu (mean)=63.7, sigma (sd)=2.7

# Make a 1000 simulations:
n_sims <- 1000
# Create an empty "shelf" to store values from the for-loop in:
store_diff <- rep(NA, n_sims)

# Create the for-loop with normal distribution of the heights of 100 men and women:
for (n in 1:n_sims){
  m_h <- rnorm(100, mean = 69.1, sd = 2.9)
  f_h <- rnorm(100, mean = 63.7, sd = 2.7)
  diff <- mean(m_h) - mean(f_h) # get the difference
  store_diff[n] <- diff # store it in the "empty shelf"
}

# Create the histogram:
hist(store_diff)

# Calculate the mean and standard deviation of the distribution of x − y:
